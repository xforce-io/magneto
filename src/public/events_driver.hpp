...
bool EventsDriver<Context, LockPolicy>::RegEvent(int fd, Op op, Event direction, Context* context, int timeo_ms) {
  ...
  switch (op) {
    case kAddEvent :
      ...
      MAG_FAIL_HANDLE(!lock_.Lock())
      ret = epoll_ctl(fd_epoll_, EPOLL_CTL_ADD, fd, &ev);
      lock_.Unlock();

      MAG_FAIL_HANDLE_FATAL(0!=ret, "fail_epoll_ctl_add fd[" << fd << "] reason[" << errno << "]")
      break;
    case kModEvent :
      if (timeo_ms>=0) { 
        event_timeo = (struct EventTimeo){Time::GetCurrentMsec(false) + timeo_ms, fd};
      ...
      MAG_FAIL_HANDLE_FATAL(0!=ret, "fail_epoll_ctl_mod fd[" << fd << "] reason[" << strerror(errno) << "]")
      break;
    default : // kDelEvent
      ret = RegEventDel(fd);
      MAG_FAIL_HANDLE(0!=ret)
...
EventsDriver<Context, LockPolicy>::~EventsDriver() { 
  ClearAll();
  MAG_DELETE_ARRAY(events_ready_) 
}

template <typename Context, typename LockPolicy>
bool EventsDriver<Context, LockPolicy>::Init_() {
  fd_epoll_ = epoll_create(1);
  MAG_FAIL_HANDLE_FATAL(fd_epoll_<=0, "error_create_fd error[" << strerror(errno) << "]")

  MAG_NEW(events_ready_, epoll_event [kMaxNumEvents])
  init_=true;
  return true;
