...
bool PoolObjsBase<Obj, LockPolicy>::Init_() {
  MAG_FAIL_HANDLE(0==max_num_blocks_)

  MAG_NEW(pool_objs_, Obj** [max_num_blocks_])
...
bool PoolObjsBase<Obj, LockPolicy>::AllocABlock_() {
  ...
  MAG_NEW(pool_objs_[num_blocks_alloc_], Obj* [num_objs_per_block_])

  for (i=0; i<num_objs_per_block_; ++i) {
    MAG_NEW(pool_objs_[current_row_][i], Obj)
  ...
  if (NULL!=pool_objs_) {
    for (size_t i=0; i<current_row_; ++i) {
      for (size_t j=0; j<num_objs_per_block_; ++j) {
        MAG_DELETE(pool_objs_[i][j]);
      }
      MAG_DELETE_ARRAY(pool_objs_[i])
    }

    if (current_row_!=num_blocks_alloc_) {
      for (size_t i=0; i<current_col_; ++i) {
        MAG_DELETE(pool_objs_[current_row_][i]);
      }
      MAG_DELETE_ARRAY(pool_objs_[current_row_])
