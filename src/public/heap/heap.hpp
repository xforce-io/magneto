...
HeapBase<Val, Order, FeatureF>::~HeapBase() {
  ...
  if (NULL!=heap_items_) {
    for (size_t i=0; i<size_container_; ++i) {
      MAG_DELETE(heap_items_[i])
    }
    MAG_FREE(heap_items_)
  }
}
...
bool HeapBase<Val, Order, FeatureF>::Init_() {
  ...
  MAG_MALLOC(heap_items_, Node**, sizeof(*heap_items_) * size_container_);
  memset(heap_items_, 0, sizeof(*heap_items_) * size_container_);
  for(size_t i=0; i<size_container_; ++i) {
    MAG_NEW(heap_items_[i], HeapItem<Val>);
  }
...
void HeapBase<Val, Order, FeatureF>::Resize_() { 
  ...
  MAG_REALLOC(newheap_items_, heap_items_, Node**, (orig_size << 1));
  for(size_t i=0; i<size_container_; ++i) {
    MAG_NEW(newheap_items_[i+size_container_], Node);
  }
