...
bool ConfNormal::Init(const std::string& filedir) {
  ...
  const JsonType* conf_normal = JsonType::CreateConf(filedir+"/"+kFilenameNormalConf);
  XFC_FAIL_HANDLE_FATAL(NULL==conf_normal || !conf_normal->IsDict(), 
  ...
  for (iter = listen_addrs->AsDict().begin();
    ...
    XFC_FAIL_HANDLE_FATAL(!ret || !iter->second.IsStr(), "invalid_listen_addr[" << iter->first << "]")
